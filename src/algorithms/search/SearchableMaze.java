...
public class SearchableMaze extends Maze implements ISearchable<MazeState> {
    ...
    public LinkedList<AState> getAllPossibleStates(MazeState origin) {
        ...
        //diagonal movement.
        if (oRow + 1 < rows && this.maze[oRow + 1][oCol] == 0) {
            if (oCol + 1 < cols && this.maze[oRow + 1][oCol + 1] == 0) open.add(new MazeState(oRow + 1, oCol + 1));
            if (oCol - 1 >= 0 && this.maze[oRow + 1][oCol - 1] == 0) open.add(new MazeState(oRow + 1, oCol - 1));
        }
        if (oRow - 1 >= 0 && this.maze[oRow - 1][oCol] == 0) {
            if (oCol + 1 < cols && this.maze[oRow - 1][oCol + 1] == 0) open.add(new MazeState(oRow - 1, oCol + 1));
            if (oCol - 1 >= 0 && this.maze[oRow - 1][oCol - 1] == 0) open.add(new MazeState(oRow - 1, oCol - 1));
        }
